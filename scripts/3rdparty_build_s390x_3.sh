...
# Set Conan local cache directory if needed
# export CONAN_USER_HOME="/root/milvuswork/.conan"    # For Conan 1.x
export CONAN_HOME="/root/milvuswork/.conan2"        # For Conan 2.x
mkdir -p "$CONAN_HOME"
echo "core.cache:storage_path=/root/milvuswork/.conan2/p" >> "$CONAN_HOME/global.conf"
...
case "${unameOut}" in
  ...
  Linux*)
    ...
    ABI_FLAG=$(gcc -v 2>&1 | sed -n 's/.*\(--with-default-libstdcxx-abi\)=\(\w*\).*/\2/p')
    if [[ "$ABI_FLAG" == "gcc4" ]]; then
      conan install ${CPP_SRC_DIR} \
        -of conan \
        --build=missing \
